...
RUN \
  ...
  mkdir -p ${DIR} && \
  cd ${DIR} && \
  curl -sL https://bitbucket.org/multicoreware/x265_git/get/master.tar.bz2 | \
  tar -xj && \
  cd multicoreware*/build/linux && \
  cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX="${PREFIX}" -DENABLE_CLI=OFF ../../source && \
  make && \
  make install && \
  rm -rf ${DIR}
RUN \
...
RUN \
        ...
        tar -zx --strip-components=1 && \
        cmake -E env CFLAGS="-DPNG_ARM_NEON_OPT=0" cmake -DBUILD_THIRDPARTY:BOOL=ON -DCMAKE_INSTALL_PREFIX="${PREFIX}" . && \
        make && \
        make install && \
        rm -rf ${DIR}
...
RUN \
	apt-get update -y && \
	apt-get install -y --no-install-recommends libva-drm2 libva2 libnuma-dev && \
